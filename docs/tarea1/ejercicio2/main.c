...
#include <stdio.h>
#include <stdlib.h> /* para usar atoi y rand*/

int main(int argc, char *argv[]) {
  if (argc != 2){
		printf ("Error! Debe ejecutarse como ./prog n...\n");
		return 1;
	}
  int	n = atoi(argv[1]);
  ...
  GLfloat points[] = { 0.0f, 1.0f, 0.0f, 
                      1.0f, -1.0f, 0.0f, 
                      -1.0f, -1.0f, 0.0f };

  GLfloat* internos = calloc(3*n, sizeof(GLfloat));

  internos[0] = 0.5f; //despues los cambiamos a random
  internos[1] = 0.5f;
  internos[2] = 0.0f;

  for(int i=0; i < n-1; i++){
    int vertice = rand() % 3; // 0
    float delta_x = internos[3*i] - points[vertice*3];
    float delta_y = internos[3*i + 1] - points[vertice*3 + 1] ;
    internos[3*i + 3] = points[vertice*3] + delta_x/2;
    internos[3*i + 4] = points[vertice*3] + delta_y/2;
    internos[3*i + 5] = 0.0f;
  }
  ...
  window = glfwCreateWindow( 640, 480, "Ejercicio 2", NULL, NULL );
  if ( !window ) {
  ...
  glBufferData( GL_ARRAY_BUFFER, 3*n* sizeof( GLfloat ), internos, GL_STATIC_DRAW );
